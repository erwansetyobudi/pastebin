...
/* Recapitulation Report With Chaching Method*/
/* by Erwan Setyo Budi
...
<?php
    ...
    // caching dimulai di sini (tidak ada else kedua)
    $recapByParam = $_GET['recapBy'] ?? 'default';
    $cache_dir = SB . 'files/cache/';
    $cache_file = $cache_dir . 'recap_' . md5($recapByParam) . '.html';
    ...
    $content = '<div class="mb-2">'.__('Title and Collection Recap by').' <strong>'.$recapby.'</strong>
        <a href="#" class="s-btn btn btn-default printReport" onclick="window.print()">'.__('Print Current Page').'</a>
        <a href="../xlsoutput.php" class="s-btn btn btn-default">'.__('Export to spreadsheet format').'</a></div>'."\n";
    $content .= $output;
